        ...
        @isset( $options )
            @if(is_array($options) && \Illuminate\Support\Arr::isAssoc($options))
                @foreach($options as $id => $option)
                    <option value="{{ $id }}"
                            @if(isset($values) && ((is_array($values) && in_array($id,$values)) || ($values instanceof \Illuminate\Database\Eloquent\Collection && $values->contains($id))) )
                            selected
                        @endif>
                        {{ $option }}
                    </option>
            ...
            @elseif($options instanceof \Illuminate\Database\Eloquent\Collection)
                @foreach($options as $option)
                    <option value="{{ $option->title ?? $option->name ?? $option->value ?? '' }}"
                            @if(isset($values) && ((is_array($values) && in_array($option->id,$values)) || ($values instanceof \Illuminate\Database\Eloquent\Collection && $values->contains($option->id))) )
                            selected
                        @endif>
